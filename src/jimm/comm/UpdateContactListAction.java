...
public class UpdateContactListAction extends Action
{
	...
	public UpdateContactListAction(ContactItem cItem, GroupItem oldGroup, GroupItem newGroup)
	{
		super(false, true);
		this.cItem = cItem;
		this.gItem = oldGroup;
	...
	protected void init() throws JimmException
	{
		...
		case ACTION_RENAME:
			...
			packet = new SnacPacket
						(
							...
							Util.getCounter(), new byte[0], buf
						);
			Icq.c.sendPacket(packet);
			state = UpdateContactListAction.STATE_CLI_ROSTERMODIFY_SENT;
			break;
		...
		case ACTION_DEL:
			if (cItem != null)
			{
				...
				state = STATE_DELETE_CONTACT1;
			}
			else
			{
				buf = packRosterItem(gItem);
				state = STATE_DELETE_GROUP1;
			}
		...
		if (packet instanceof SnacPacket)
		{
					...
					&& (snacPacket.getCommand() == SnacPacket.SRV_UPDATEACK_COMMAND))
			{
				...
				case STATE_DELETE_GROUP1:
					sendGroupsList();
					state = STATE_DELETE_GROUP2;
					break;

				case STATE_DELETE_GROUP2:
					ContactList.removeGroup(gItem);
				...
				case STATE_ADD_GROUP1:
					sendGroupsList();
					state = STATE_ADD_GROUP2;
					break;

				case STATE_ADD_GROUP2:
					ContactList.addGroup(gItem);
					sendCLI_ADDEND();
					state = STATE_COMPLETED;
					break;
				...
				case STATE_CLI_ROSTERMODIFY_SENT:
					if (action != ACTION_RENAME) sendCLI_ADDEND();
				...
				case STATE_DELETE_CONTACT1:
					GroupItem group = ContactList.getGroupById(cItem.getIntValue(ContactItem.CONTACTITEM_GROUP));
				...
				if ((cItem != null) && (action == ACTION_DEL) && (cItem.getIntValue(ContactItem.CONTACTITEM_GROUP) == 0))
				{
					state = STATE_COMPLETED;
				}
			}
	...
	static private void sendGroup(GroupItem group) throws JimmException
	{
		try
		{
			Icq.c.sendPacket
	...
	private byte[] packRosterItem(ContactItem cItem, int groupID)
	{
		...
		/* Name */
		Util.writeLenAndString(stream, cItem.getStringValue(ContactItem.CONTACTITEM_UIN), true);
	...
	static private byte[] packRosterItem(GroupItem gItem)
	{
		ByteArrayOutputStream stream = new ByteArrayOutputStream();
