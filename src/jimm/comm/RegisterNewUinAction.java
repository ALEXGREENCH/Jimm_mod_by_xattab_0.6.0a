...
public class RegisterNewUinAction extends Action
{
	...
	protected void init() throws JimmException
	{
		...
		// Set STATE_INIT
		state = RegisterNewUinAction.STATE_INIT_DONE;
	...
	try
	{
		...
		switch (this.state)
		{
			...
			case RegisterNewUinAction.STATE_WAITING_FOR_CONFIRM :
				if (packet instanceof SnacPacket)
				{
					...
					else if ((snacPacket.getFamily() == 0x0017) && (snacPacket.getCommand() == 0x0001))
					{
						// Move to next state
						this.state = RegisterNewUinAction.STATE_WAITING_GOODBYE;
						// Packet has been consumed
		...
		catch (JimmException e)
		{
			...
			if (e.isCritical()) state = RegisterNewUinAction.STATE_ERROR;
	...
	{
		return (state == RegisterNewUinAction.STATE_WELL_DONE);
	}

	// Returns true if an error has occured
	public boolean isError()
	{
		return (state == RegisterNewUinAction.STATE_ERROR);
	}
	...
	public void onEvent(int eventType)
	{
		...
		{
		case ON_COMPLETE:
			Options.submitNewUinPassword(getUin(), getPassword());
			Icq.disconnect();
			break;
